...
query2 = '''
SELECT nim, nama_mhs, nama_prodi FROM mahasiswa INNER JOIN prodi ON prodi.id = mahasiswa.id_prodi LIMIT 1000
'''
...
df.to_csv('percobaan4.csv', mode='a', header=False, index=False)

# menampilkan hasil percobaan
data = pd.read_csv('percobaan4.csv', names=['time'])
